...
export default {
  methods: {
    createMesh(geom, imageFile) {
      ...
      var texture = loader.load(this.$withBase("/assets/textures/seafloor.dds"))
    ...
    init() {
      ...
      var polyhedron = this.createMesh(new THREE.IcosahedronGeometry(5, 0))
      polyhedron.position.x = 12
      scene.add(polyhedron)
      var sphere = this.createMesh(new THREE.SphereGeometry(5, 20, 20))
      scene.add(sphere)
      var cube = this.createMesh(new THREE.BoxGeometry(5, 5, 5))
      cube.position.x = -12
      scene.add(cube)
      camera.position.x = 0
...
export default {
  methods: {
    createMesh(geom, imageFile) {
      ...
      var texture = loader.load(this.$withBase("/assets/textures/tex_base.pvr"))
      var mat = new THREE.MeshPhongMaterial()
      mat.map = texture
  ...
  methods: {
    createMesh(geom, imageFile) {
      ...
      var texture = loader.load(
        this.$withBase("/assets/textures/crate_color8.tga")
      )
      var mat = new THREE.MeshPhongMaterial()
      mat.map = texture
  ...
  methods: {
    createMesh(geom, imageFile) {
      var texture = THREE.ImageUtils.loadTexture(
        this.$withBase("/assets/textures/general/" + imageFile)
      )
      var mat = new THREE.MeshPhongMaterial()
      mat.map = texture
    ...
    init() {
      ...
      var polyhedron = this.createMesh(
      ...
      var sphere = this.createMesh(
        new THREE.SphereGeometry(5, 20, 20),
        "floor-wood.jpg"
      )
      scene.add(sphere)

      var cube = this.createMesh(
    ...
    createMesh(geom, imageFile, bump) {
      ...
      if (bump) {
        var bump = THREE.ImageUtils.loadTexture(
          this.$withBase("/assets/textures/general/" + bump)
        )
        mat.bumpMap = bump
        mat.bumpScale = 0.2
        console.log("d")
    ...
    init() {
      ...
      var controls = new (function () {
        ...
        this.changeTexture = function (e) {
          var texture = THREE.ImageUtils.loadTexture(
            this.$withBase("/assets/textures/general/" + e + ".jpg")
          )
          sphere2.material.map = texture
          sphere1.material.map = texture

          var bump = THREE.ImageUtils.loadTexture(
            this.$withBase("/assets/textures/general/" + e + "-bump.jpg")
          )
    ...
    createMesh(geom, imageFile, normal) {
      if (normal) {
        ...
        var m = THREE.ImageUtils.loadTexture(
          this.$withBase("/assets/textures/general/" + normal)
        )
    ...
    init() {
      ...
      var controls = new (function () {
        ...
        this.changeTexture = function (e) {
          ...
          var bump = THREE.ImageUtils.loadTexture(
            this.$withBase("/assets/textures/general/" + e + "-normal.jpg")
          )
  ...
  methods: {
    init() {
      ...
      var lm = THREE.ImageUtils.loadTexture(
        this.$withBase("/assets/textures/lightmap/lm-1.png")
      )
      ...
      meshMaterial.map = THREE.ImageUtils.loadTexture(
        this.$withBase("/assets/textures/general/stone.jpg")
      )
...
export default {
  methods: {
    ...
    createCubeMap() {
      var path = this.$withBase("/assets/textures/cubemap/parliament/")
      var format = ".jpg"
      var urls = [
        path + "posx" + format,
        path + "negx" + format,
  ...
  methods: {
    ...
    geometry() {
      return {
        ...
        sphere1: new THREE.SphereGeometry(10, 15, 15),
        sphere2: new THREE.BoxGeometry(10, 15, 15),
        sphereLight: new THREE.SphereGeometry(0.2),
      }
    },
    // 材质
    material(textureCube) {
      var shader = THREE.ShaderLib["cube"]
      shader.uniforms["tCube"].value = textureCube
      return {
    ...
    model(textureCube) {
      ...
      var material = this.material(textureCube)
      var cube = new THREE.Mesh(geometry.cube, material.cube)
      var sphere1 = this.createMesh(geometry.sphere1, "plaster.jpg")
      sphere1.material.envMap = textureCube
      sphere1.rotation.set(-0.5, 12, 5)
      var sphere2 = this.createMesh(
        geometry.sphere2,
        "plaster.jpg",
        "plaster-normal.jpg"
      ...
      sphere2.rotation.set(0.5, -12, 5)
      var sphereLightMesh = new THREE.Mesh(
        geometry.sphereLight,
        material.sphereLightMaterial
      )
      sphereLightMesh.castShadow = true
      ...
      light.position.set(0, 30, 30)
      light.intensity = 1.2
      var directionalLight = new THREE.PointLight("#ff5808")
    ...
    createMesh(geom, texture, normal) {
      geom.computeVertexNormals()

      if (normal) {
        var t = THREE.ImageUtils.loadTexture(
          this.$withBase("/assets/textures/general/" + texture)
        )
        var m = THREE.ImageUtils.loadTexture(
          this.$withBase("/assets/textures/general/" + normal)
        )
    ...
    init() {
      ...
      var textureCube = this.createCubeMap()
      var model = this.model(textureCube)
      var scene = this.scene({ model })
      sceneCube.add(model.cube)
      camera.lookAt(new THREE.Vector3(0, 0, 0))
      this.element(webGLRenderer.domElement)
      var step = 0
      var controls = this.controls(model.sphere1, model.sphere2)
      this.gui(controls)
      render()
      var invert = 1
      var phase = 0
      ...
      function render() {
        ...
        step += 0.1
        if (controls.rotate) {
          model.sphere1.rotation.y -= 0.01
          model.sphere2.rotation.y += 0.01
        }
        if (phase > 2 * Math.PI) {
          invert = invert * -1
          phase -= 2 * Math.PI
        ...
        model.sphereLightMesh.position.z = +(21 * Math.sin(phase))
        model.sphereLightMesh.position.x = -14 + 14 * Math.cos(phase)

        if (invert < 0) {
          var pivot = 0
          model.sphereLightMesh.position.x =
            invert * (model.sphereLightMesh.position.x - pivot) + pivot
        }
        model.directionalLight.position.copy(model.sphereLightMesh.position)
...
export default {
  methods: {
    init() {
      ...
      var texture = THREE.ImageUtils.loadTexture(
        this.$withBase("/assets/textures/ash_uvgrid01.jpg")
      )
...
export default {
  methods: {
    init() {
      ...
      // setup the control gui
      let img1 = this.$withBase("/assets/models/UVCube1.obj")
      let img2 = this.$withBase("/assets/textures/ash_uvgrid01.jpg")
      let img3 = this.$withBase("/assets/models/UVCube2.obj")
      var controls = new (function () {
        // we need the first child, since it's a multimaterial
        this.loadCube1 = function () {
          var loader = new THREE.OBJLoader()
          loader.load(img1, function (geometry) {
            if (mesh) scene.remove(mesh)
            var material = new THREE.MeshBasicMaterial({ color: 0xffffff })
            var texture = THREE.ImageUtils.loadTexture(img2)
        ...
        this.loadCube2 = function () {
          var loader = new THREE.OBJLoader()
          loader.load(img3, function (geometry) {
            if (mesh) scene.remove(mesh)
            var material = new THREE.MeshBasicMaterial({ color: 0xffffff })
            var texture = THREE.ImageUtils.loadTexture(img2)
            material.map = texture
...
export default {
  methods: {
    init() {
      ...
      this.$refs.example.appendChild(canvas)
      // this.$refs.example.literallycanvas({
      //   imageURLPrefix: this.$withBase("/libs/literally/img"),
      // })
      var stats = this.initStats()
  ...
  methods: {
    init() {
      ...
      function init() {
        ...
        webGLRenderer.shadowMapEnabled = true
        var png = this.$withBase("/assets/textures/general/floor-wood.jpg")
        var cube = createMesh(new THREE.BoxGeometry(12, 12, 12), png)
        cube.position.x = 3
        scene.add(cube)
...
<template>
  <div style="position:relative">
    ...
    <video
      ...
      src='$withBase("/assets/movies/Big_Buck_Bunny_small.ogv")'
      controls="true"
      autoplay="true"
    ></video>
    <div ref="example"></div>
...
export default {
  ...
  methods: {
    init() {
      ...
      cube = createMesh(new THREE.BoxGeometry(22, 16, 0.2), texture)
      cube.position.y = 2
      scene.add(cube)
